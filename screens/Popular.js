...
const PopularRecipes = () => {
  ...
  useEffect(() => {
    fetchAllPopularRecipes();
  }, []);

  const fetchAllPopularRecipes = async () => {
    try {
      const response = await axios.get(
        'https://www.themealdb.com/api/json/v1/1/search.php?s='
      );

      const popularRecipesData = response.data.meals || [];
  ...
  const totalPages = Math.ceil(popularRecipes.length / recipesPerPage);
  ...
  const currentRecipes = popularRecipes.slice(startIndex, endIndex);
