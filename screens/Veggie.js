...
const VegetableRecipes = () => {
  ...
  useEffect(() => {
    fetchAllVegetableRecipes();
  }, []);

  const fetchAllVegetableRecipes = async () => {
    try {
      const response = await axios.get(
        'https://www.themealdb.com/api/json/v1/1/filter.php?c=Vegetarian'
      );

      const vegetableRecipesData = response.data.meals || [];

      const detailedVegetableRecipes = await Promise.all(
        vegetableRecipesData.map(async (recipe) => {
      ...
      setVegetableRecipes(detailedVegetableRecipes);
    } catch (error) {
  ...
  const totalPages = Math.ceil(vegetableRecipes.length / recipesPerPage);
  ...
  const currentRecipes = vegetableRecipes.slice(startIndex, endIndex);

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Vegetable Recipes</Text>
      <ScrollView>
...
export default VegetableRecipes;
