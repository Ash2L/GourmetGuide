...
const ChickenRecipes = () => {
  ...
  useEffect(() => {
    fetchAllChickenRecipes();
  }, []);

  const fetchAllChickenRecipes = async () => {
    try {
      const response = await axios.get(
        'https://www.themealdb.com/api/json/v1/1/filter.php?c=Chicken'
      );

      const chickenRecipesData = response.data.meals || [];

      const detailedChickenRecipes = await Promise.all(
        chickenRecipesData.map(async (recipe) => {
      ...
      setChickenRecipes(detailedChickenRecipes);
    } catch (error) {
  ...
  const totalPages = Math.ceil(chickenRecipes.length / recipesPerPage);
  ...
  const currentRecipes = chickenRecipes.slice(startIndex, endIndex);

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Chicken Recipes</Text>
      <ScrollView>
