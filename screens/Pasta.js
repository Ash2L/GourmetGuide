...
const PastaRecipes = () => {
  ...
  useEffect(() => {
    fetchAllPastaRecipes();
  }, []);

  const fetchAllPastaRecipes = async () => {
    try {
      const response = await axios.get(
        'https://www.themealdb.com/api/json/v1/1/filter.php?c=Pasta'
      );

      const pastaRecipesData = response.data.meals || [];

      const detailedPastaRecipes = await Promise.all(
        pastaRecipesData.map(async (recipe) => {
      ...
      setPastaRecipes(detailedPastaRecipes);
    } catch (error) {
  ...
  const totalPages = Math.ceil(pastaRecipes.length / recipesPerPage);
  ...
  const currentRecipes = pastaRecipes.slice(startIndex, endIndex);

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Pasta Recipes</Text>
      <ScrollView>
