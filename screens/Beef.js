...
const BeefRecipes = () => {
  ...
  const fetchAllBeefRecipes = async () => {
    try {
      const response = await axios.get(
        'https://www.themealdb.com/api/json/v1/1/filter.php?c=Beef'
      );

      const beefRecipesData = response.data.meals || [];

      const detailedBeefRecipes = await Promise.all(
        beefRecipesData.map(async (recipe) => {
      ...
      setBeefRecipes(detailedBeefRecipes);
    } catch (error) {
  ...
  const totalPages = Math.ceil(beefRecipes.length / recipesPerPage);
  ...
  const currentRecipes = beefRecipes.slice(startIndex, endIndex);

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Beef Recipes</Text>
      <ScrollView>
